...
export function recordCroEvent(db: Database, input: z.infer<typeof croEventSchema>) {
  ...
  db.prepare(`INSERT INTO cro_events (id,site_id,visitor_id,session_id,event_name,occurred_at,page,referrer,device_type,utm_source,utm_medium,utm_campaign,utm_term,utm_content,properties_json)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(randomUUID(), event.siteId, event.visitorId, event.sessionId, event.eventName, nowIso(), event.page, event.referrer || null, event.deviceType, event.utm.source ?? null, event.utm.medium ?? null, event.utm.campaign ?? null, event.utm.term ?? null, event.utm.content ?? null, JSON.stringify(event.properties));
}
...
export function croDashboard(db: Database, siteId: string) {
  ...
  const stageRows = stages.map((stage, index) => {
    ...
    return { ...stage, dropoff, dropoffRate:previous?.count ? Math.round(dropoff * 1000 / previous.count) / 10 : 0 };
  });
  ...
  const homepage = {
    ...
    ctas:ctaRows.map(row=>({...row, rate:homepageSessions ? Math.round(row.clicks*1000/homepageSessions)/10 : 0})),
  ...
  return { visitors, sessions, conversions, conversionRate:sessions ? Math.round(conversions*1000/sessions)/10 : 0, stages:stageRows, devices, sources, homepage, insights };
}
